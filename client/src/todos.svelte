<script>
	...
	fetch("https://saml-multitenancy.herokuapp.com//api/v1/todo/", {
		headers: {
		...
		.then((result) => {
			...
			for (let todo of result.todos) {
				newTodos.push({ id: todo._id, text: todo.text, done: todo.done });
			}
	...
	function removeTask(event) {
		...
		fetch(`https://saml-multitenancy.herokuapp.com//api/v1/todo/?id=${id}`, {
			method: "DELETE",
			headers: {
				Authorization: localStorage.getItem("token"),
			},
	...
	function editTask(event) {
		...
		fetch(
			`https://saml-multitenancy.herokuapp.com//api/v1/todo/updatetext/?id=${id}`,
			{
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
